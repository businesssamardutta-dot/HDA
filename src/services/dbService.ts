...
const STORAGE_KEY = 'haribansho_db_v1';
    ...
    assignments: [
      {
        ...
        assigned_at: '2025-05-17T09:50:00Z',
        completed_at: '2025-05-17T10:30:00Z',
      ...
      },
      {
        id: 'asg-2',
        order_id: 'ord-1249',
        delivery_boy_id: 'db-2',
        delivery_boy_name: 'Suresh Yadav',
        assigned_by: 'Super Admin',
        assignment_status: 'On The Way',
        assigned_at: '2025-05-17T09:35:00Z',
        accepted_at: '2025-05-17T09:40:00Z',
        created_at: '2025-05-17T09:35:00Z',
        updated_at: '2025-05-17T09:40:00Z',
      },
      {
        id: 'asg-3',
        order_id: 'ord-1248',
...
function saveLocalDB(state: LocalDBState) {
  try {
    ...
    // Trigger storage event for cross-component realtime sync
...
export const dbService = {
  ...
  async getDashboardStats(): Promise<DashboardStats> {
    ...
    const totalOrders = 1248; // Baseline from reference dashboard with live addition
    const pendingOrders = orders.filter(o => o.order_status === 'Pending').length + 155;
    const assignedOrders = orders.filter(o => o.order_status === 'Assigned' || o.order_status === 'Out for Delivery').length + 530;
    const deliveredOrders = orders.filter(o => o.order_status === 'Delivered').length + 1030;
    const cancelledOrders = orders.filter(o => o.order_status === 'Cancelled').length + 64;

    const baseRevenue = 245890;
    const additionalRevenue = orders.reduce((sum, o) => sum + (o.order_status === 'Delivered' ? o.total_amount : 0), 0);

    return {
      totalOrders: totalOrders + orders.length - 5,
      totalOrdersGrowth: 18.5,
      pendingOrders,
      pendingOrdersGrowth: 12.3,
      assignedOrders,
      assignedOrdersGrowth: 15.8,
      deliveredOrders,
      deliveredOrdersGrowth: 20.4,
      cancelledOrders,
      cancelledOrdersGrowth: -5.6,
      totalRevenue: baseRevenue + additionalRevenue,
      totalRevenueGrowth: 22.7,
      todayNewOrders: 245,
      todayOutForDelivery: 320,
      todayDelivered: 186,
      todayCodAmount: 125600,
  ...
  async createOrder(orderData: Partial<Order> & { items: any[] }): Promise<Order> {
    const db = loadLocalDB();
    const orderNumber = `#ORD${1251 + db.orders.length - 5}`;
    ...
    const newOrder: Order = {
      ...
      delivery_address_id: orderData.delivery_address_id || 'addr-1',
      delivery_address_text: orderData.delivery_address_text || 'Lucknow Hub',
      zone_id: orderData.zone_id || 'zone-1',
      zone_name: orderData.zone_name || 'North Zone',
      ...
      delivery_charge: orderData.delivery_charge || 40,
      tax_amount: orderData.tax_amount || 0,
      total_amount: orderData.total_amount || 0,
      cod_amount: orderData.payment_method === 'COD' ? (orderData.total_amount || 0) : 0,
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        await supabase.from('01_orders').insert([newOrder]);
      } catch (e) {
        console.warn('Supabase insert failed, stored in local database', e);
      }
    }
  ...
  async getDeliveryBoys(): Promise<DeliveryBoy[]> {
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        if (!error && data && data.length > 0) return data as DeliveryBoy[];
      } catch (e) {}
    }
  ...
  async addDeliveryBoy(boyData: Partial<DeliveryBoy>): Promise<DeliveryBoy> {
    ...
    const newBoy: DeliveryBoy = {
      id: `db-${Date.now()}`,
      employee_code: `EMP-${100 + db.deliveryBoys.length + 1}`,
      first_name: boyData.first_name || 'New',
      last_name: boyData.last_name || 'Driver',
      full_name: `${boyData.first_name || 'New'} ${boyData.last_name || 'Driver'}`,
      phone: boyData.phone || '+91 98000 00000',
      email: boyData.email || 'driver@haribansho.com',
      profile_image_url: boyData.profile_image_url || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&auto=format&fit=crop&q=80',
      ...
      last_location_name: 'Lucknow Central',
  ...
  async addProduct(productData: Partial<Product>): Promise<Product> {
    ...
    const newProd: Product = {
      id: `prod-${Date.now()}`,
      product_code: `PRD-${100 + db.products.length + 1}`,
      name: productData.name || 'New Product',
      slug: (productData.name || 'product').toLowerCase().replace(/\s+/g, '-'),
      description: productData.description || '',
      category_id: productData.category_id || 'cat-1',
      category_name: productData.category_name || 'Groceries & Staples',
      sku: productData.sku || `SKU-${Date.now().toString().slice(-6)}`,
      barcode: productData.barcode || '8901234567890',
      unit: productData.unit || 'pack',
      selling_price: productData.selling_price || 100,
      cost_price: productData.cost_price || 80,
      tax_percentage: productData.tax_percentage || 5,
      image_url: productData.image_url || 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=200&auto=format&fit=crop&q=80',
      quantity_available: productData.quantity_available || 50,
  ...
  async addCategory(catData: Partial<Category>): Promise<Category> {
    ...
    const newCat: Category = {
      ...
      image_url: catData.image_url || 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=150&auto=format&fit=crop&q=80',
      is_active: true,
  ...
  async addVehicle(vehData: Partial<Vehicle>): Promise<Vehicle> {
    ...
    const newVeh: Vehicle = {
      id: `veh-${Date.now()}`,
      vehicle_number: vehData.vehicle_number || `UP32-XX-${Math.floor(1000 + Math.random() * 9000)}`,
      vehicle_type: vehData.vehicle_type || 'Bike',
      brand: vehData.brand || 'Honda',
      model: vehData.model || 'Activa 6G',
      fuel_type: vehData.fuel_type || 'Petrol',
      capacity: vehData.capacity || '25 kg',
  ...
  async sendNotification(notifData: Partial<AppNotification>): Promise<AppNotification> {
    ...
    const newNotif: AppNotification = {
      id: `notif-${Date.now()}`,
      title: notifData.title || 'System Broadcast',
      message: notifData.message || 'Notification broadcast to team',
  ...
  async addCoupon(couponData: Partial<Coupon>): Promise<Coupon> {
    ...
    const newCoupon: Coupon = {
      id: `coup-${Date.now()}`,
      code: (couponData.code || 'SALE10').toUpperCase(),
      description: couponData.description || 'Special promo coupon',
      discount_type: couponData.discount_type || 'percentage',
      discount_value: couponData.discount_value || 10,
      minimum_order_amount: couponData.minimum_order_amount || 299,
      maximum_discount_amount: couponData.maximum_discount_amount || 100,
      usage_limit: couponData.usage_limit || 500,
      usage_count: 0,
      per_customer_limit: couponData.per_customer_limit || 1,
      start_date: couponData.start_date || new Date().toISOString().split('T')[0],
      end_date: couponData.end_date || '2025-12-31',
      is_active: true,
      created_at: now,
      updated_at: now,
    };
  ...
  async createSupportTicket(ticketData: Partial<SupportTicket>): Promise<SupportTicket> {
    ...
    const newTicket: SupportTicket = {
      id: `tkt-${Date.now()}`,
      ticket_number: `TKT-${Math.floor(8000 + Math.random() * 1999)}`,
